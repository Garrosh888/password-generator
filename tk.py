...
with open("my passwords.txt") as file:
    data = file.readlines()
text_pass = ""
text_desk = ""#переменная для хронения описане пароля
text_date = ""
is_password = True
...
for stroka in data:
    ...
    for symbol in stroka:
        ...
        if is_password == False and is_description == True:
            text_desk = text_desk + symbol
        if is_password == False and is_description == False:
            text_date = text_date + symbol
        if symbol == " " and is_password == False and stroka[index -1 ] == "-" and stroka[index -2 ] == " ":
            is_description = True
        if symbol == " " and is_password == True:
            is_password = False
    if len(text_pass) > 3:
        save_passwords[text_pass] = {text_date:text_desk}
    text_pass = ""
    text_desk = ""
    text_date = ""
